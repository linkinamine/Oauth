...
import static com.befriender.lib.Consts.TAG;
...
public class TwitterSocialNetwork extends OAuthSocialNetwork {
	...
	private final String TWITTER_CALLBACK_URL = "oauth://befriender";
	...
	public TwitterSocialNetwork(Fragment fragment, String consumerKey,
		...
		if (TextUtils.isEmpty(fConsumerKey)
				|| TextUtils.isEmpty(fConsumerSecret)) {
			throw new IllegalArgumentException(
					"consumerKey and consumerSecret are invalid");
		}
	...
	private void initTwitterClient() {
		...
		if (TextUtils.isEmpty(accessToken)
			...
			mTwitter = factory.getInstance(new AccessToken(accessToken,
					accessTokenSecret));
		}
	}
	...
	@Override
	public com.befriender.lib.AccessToken getAccessToken() {
		return new com.befriender.lib.AccessToken(mSharedPreferences.getString(
				SAVE_STATE_KEY_OAUTH_TOKEN, null),
	...
	public void requestCurrentPerson(
		...
		executeRequest(new RequestGetPersonAsyncTask(), null,
	...
	public void requestSocialPerson(
		...
		try {
			args.putLong(RequestGetPersonAsyncTask.PARAM_USER_ID,
		...
		executeRequest(new RequestGetPersonAsyncTask(), args,
				REQUEST_GET_PERSON);
	}
	...
	public void requestPostMessage(String message,
		...
		executeRequest(new RequestUpdateStatusAsyncTask(), args,
	...
	public void requestPostPhoto(File photo, String message,
		...
		Bundle args = new Bundle();
		args.putString(RequestUpdateStatusAsyncTask.PARAM_MESSAGE, message);
		args.putString(RequestUpdateStatusAsyncTask.PARAM_PHOTO_PATH,
	...
	public void requestCheckIsFriend(String userID,
		...
		Bundle args = new Bundle();
		try {
			args.putLong(RequestCheckIsFriendAsyncTask.PARAM_USER_ID,
					Long.parseLong(userID));
		} catch (NumberFormatException e) {
			Log.e(TAG, "ERROR", e);
			throw new SocialNetworkException("userID should be long number");
	...
	public void requestAddFriend(
		...
		Bundle args = new Bundle();
		try {
			args.putLong(RequestAddFriendAsyncTask.PARAM_USER_ID,
					Long.parseLong(userID));
		} catch (NumberFormatException e) {
			Log.e(TAG, "ERROR", e);
			throw new SocialNetworkException("userID should be long number");
		...
		executeRequest(new RequestAddFriendAsyncTask(), args,
				REQUEST_ADD_FRIEND);
	}
	...
	public void requestRemoveFriend(
		...
		Bundle args = new Bundle();
		try {
			args.putLong(RequestRemoveFriendAsyncTask.PARAM_USER_ID,
					Long.parseLong(userID));
		} catch (NumberFormatException e) {
			Log.e(TAG, "ERROR", e);
			throw new SocialNetworkException("userID should be long number");
		...
		executeRequest(new RequestRemoveFriendAsyncTask(), args,
		...
		if (uri != null && uri.toString().startsWith(TWITTER_CALLBACK_URL)) {
			...
			if (mLocalListeners.get(REQUEST_LOGIN) != null) {
				mLocalListeners.get(REQUEST_LOGIN).onError(getID(),
						REQUEST_LOGIN, "incorrect URI returned: " + uri, null);
	...
	private class RequestLoginAsyncTask extends SocialNetworkAsyncTask {
		...
		protected Bundle doInBackground(Bundle... params) {
			...
			try {
				...
				Uri oauthLoginURL = Uri.parse(mRequestToken
		...
		protected void onPostExecute(Bundle result) {
			...
			if (result.containsKey(RESULT_OAUTH_LOGIN)) {
				Intent intent = new Intent(mSocialNetworkManager.getActivity(),
						OAuthActivity.class).putExtra(
						OAuthActivity.PARAM_CALLBACK, TWITTER_CALLBACK_URL)
						.putExtra(OAuthActivity.PARAM_URL_TO_LOAD,
								result.getString(RESULT_OAUTH_LOGIN));

				mSocialNetworkManager.startActivityForResult(intent,
						REQUEST_AUTH);
			}
		}
	}
		...
		protected void onPostExecute(Bundle result) {
			...
			mSharedPreferences
					...
					.putString(SAVE_STATE_KEY_OAUTH_SECRET,
							result.getString(RESULT_SECRET))
					.putLong(SAVE_STATE_KEY_USER_ID,
							result.getLong(RESULT_USER_ID)).apply();
	...
	private class RequestGetPersonAsyncTask extends SocialNetworkAsyncTask {
		...
		protected Bundle doInBackground(Bundle... params) {
			...
			try {
				...
				result.putString(RESULT_AVATAR_URL,
						user.getBiggerProfileImageURL());
				result.putString(RESULT_PROFILE_URL, "https://twitter.com/"
						+ user.getScreenName());
	...
	private class RequestUpdateStatusAsyncTask extends SocialNetworkAsyncTask {
		...
		@Override
		protected void onPostExecute(Bundle result) {
			String requestID = result.getBoolean(RESULT_POST_PHOTO) ? REQUEST_POST_PHOTO
					: REQUEST_POST_MESSAGE;

			mRequests.remove(requestID);

			String error = result.containsKey(RESULT_ERROR) ? result
					.getString(RESULT_ERROR) : null;
	...
	private class RequestCheckIsFriendAsyncTask extends SocialNetworkAsyncTask {
		...
		protected Bundle doInBackground(Bundle... params) {
			...
			try {
				long currentUserID = mSharedPreferences.getLong(
						SAVE_STATE_KEY_USER_ID, -1);

				Relationship relationship = mTwitter.showFriendship(
						currentUserID, userID);
				result.putBoolean(RESULT_IS_FRIEND,
		...
		protected void onPostExecute(Bundle result) {
			...
			((OnCheckIsFriendCompleteListener) mLocalListeners
					...
					getID(), "" + result.getLong(RESULT_REQUESTED_ID),
	...
	private class RequestRemoveFriendAsyncTask extends SocialNetworkAsyncTask {
		...
		protected void onPostExecute(Bundle result) {
			...
			((OnRequestRemoveFriendCompleteListener) mLocalListeners
					.get(REQUEST_REMOVE_FRIEND)).onRequestRemoveFriendComplete(
					getID(), "" + result.getLong(RESULT_REQUESTED_ID));
		}
	}
