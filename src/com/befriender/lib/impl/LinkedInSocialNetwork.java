...
import com.befriender.lib.OAuthActivity;
import com.befriender.lib.OAuthSocialNetwork;
import com.befriender.lib.SocialNetworkAsyncTask;
...
public class LinkedInSocialNetwork extends OAuthSocialNetwork {
	...
	private static final String TAG = LinkedInSocialNetwork.class
			.getSimpleName();
	...
	private static final EnumSet<ProfileField> PROFILE_PARAMETERS = EnumSet
	...
	public LinkedInSocialNetwork(Fragment fragment, String consumerKey,
			String consumerSecret, String permissions) {
		super(fragment);
		Log.d(TAG, "new LinkedInSocialNetwork: " + consumerKey + " : "
				+ consumerSecret + " : " + permissions);

		if (TextUtils.isEmpty(consumerKey) || TextUtils.isEmpty(consumerSecret)
				|| TextUtils.isEmpty(permissions)) {
			throw new IllegalArgumentException(
					"TextUtils.isEmpty(fConsumerKey) || TextUtils.isEmpty(fConsumerSecret) || TextUtils.isEmpty(fPermissions)");
		}

		mOAuthService = LinkedInOAuthServiceFactory.getInstance()
				.createLinkedInOAuthService(consumerKey, consumerSecret);
		mLinkedInApiClientFactory = LinkedInApiClientFactory.newInstance(
	...
	public boolean isConnected() {
		String accessToken = mSharedPreferences.getString(
				SAVE_STATE_KEY_OAUTH_TOKEN, null);
		String accessTokenSecret = mSharedPreferences.getString(
				SAVE_STATE_KEY_OAUTH_SECRET, null);
		return accessToken != null && accessTokenSecret != null;
	}

	@Override
	public AccessToken getAccessToken() {
		return new AccessToken(mSharedPreferences.getString(
				SAVE_STATE_KEY_OAUTH_TOKEN, null),
	...
	public void requestCurrentPerson(
		...
		executeRequest(new RequestCurrentPersonAsyncTask(), null,
	...
	public void requestSocialPerson(
		...
		executeRequest(new RequestSocialPersonAsyncTask(), args,
				REQUEST_GET_PERSON);
	}
	...
	public void requestPostMessage(String message,
		...
		executeRequest(new RequestPostMessageAsyncTask(), args,
	...
	public void requestCheckIsFriend(String userID,
		...
		executeRequest(new RequestCheckIsFriendAsyncTask(), args,
	...
	public void requestAddFriend(
		...
		executeRequest(new RequestSendInviteAsyncTask(), args,
				REQUEST_ADD_FRIEND);
	}
	...
	public void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		if (resultCode != Activity.RESULT_OK || data == null
			...
			if (mLocalListeners.get(REQUEST_LOGIN) != null) {
				mLocalListeners.get(REQUEST_LOGIN).onError(getID(),
						REQUEST_LOGIN, "Login canceled", null);
			}
		...
		args.putString(RequestLogin2AsyncTask.PARAM_AUTH_REQUEST_TOKEN,
				mOAuthTokenSecret);
		executeRequest(new RequestLogin2AsyncTask(), args, REQUEST_LOGIN2);
	}

	private void fatalError() {
		mSharedPreferences.edit().remove(SAVE_STATE_KEY_OAUTH_TOKEN)
				.remove(SAVE_STATE_KEY_OAUTH_SECRET).apply();
		((OnLoginCompleteListener) mLocalListeners.get(REQUEST_LOGIN))
				.onLogOutSuccess(getID());
	}
	...
	private class RequestLoginAsyncTask extends SocialNetworkAsyncTask {
		...
		protected Bundle doInBackground(Bundle... params) {
			...
			try {
				final LinkedInRequestToken liToken = mOAuthService
		...
		protected void onPostExecute(Bundle result) {
			...
			Intent intent = new Intent(mSocialNetworkManager.getActivity(),
					OAuthActivity.class).putExtra(OAuthActivity.PARAM_CALLBACK,
					OAUTH_CALLBACK_URL).putExtra(
					OAuthActivity.PARAM_URL_TO_LOAD,
	...
	private class RequestLogin2AsyncTask extends SocialNetworkAsyncTask {
		...
		protected Bundle doInBackground(Bundle... params) {
			...
			final String paramAuthRequestToken = args
					.getString(PARAM_AUTH_REQUEST_TOKEN);
			...
			try {
				final LinkedInAccessToken accessToken = mOAuthService
						.getOAuthAccessToken(
		...
		protected void onPostExecute(Bundle result) {
			...
			mSharedPreferences
					.edit()
					.putString(SAVE_STATE_KEY_OAUTH_TOKEN,
							result.getString(RESULT_TOKEN))
					.putString(SAVE_STATE_KEY_OAUTH_SECRET,
							result.getString(RESULT_SECRET)).apply();
	...
	private class RequestCurrentPersonAsyncTask extends SocialNetworkAsyncTask {
		...
		protected Bundle doInBackground(Bundle... params) {
			...
			try {
				LinkedInApiClient client = mLinkedInApiClientFactory
						.createLinkedInApiClient(new LinkedInAccessToken(
								mSharedPreferences.getString(
										SAVE_STATE_KEY_OAUTH_TOKEN, null),
								mSharedPreferences.getString(
										SAVE_STATE_KEY_OAUTH_SECRET, null)));
				Person person = client
						.getProfileForCurrentUser(PROFILE_PARAMETERS);

				result.putString(RESULT_ID, person.getId());
				result.putString(RESULT_NAME, person.getFirstName() + " "
						+ person.getLastName());
				result.putString(RESULT_AVATAR_URL, person.getPictureUrl());
				result.putString(RESULT_PROFILE_URL,
				...
				if (positions.size() > 0) {
					...
					result.putString(RESULT_COMPANY, position.getCompany()
							.getName());
	...
	private class RequestSocialPersonAsyncTask extends SocialNetworkAsyncTask {
		...
		protected Bundle doInBackground(Bundle... params) {
			...
			try {
				...
				List<Position> positions = person.getPositions() == null ? new ArrayList<Position>()
						: person.getPositions().getPositionList();
				if (positions.size() > 0) {
					Position position = positions.get(positions.size() - 1);
		...
		protected void onPostExecute(Bundle result) {
			...
			((OnRequestSocialPersonCompleteListener) mLocalListeners
					.get(REQUEST_GET_PERSON)).onRequestSocialPersonSuccess(
					getID(), socialPerson);
	...
	private class RequestPostMessageAsyncTask extends SocialNetworkAsyncTask {
		...
		protected Bundle doInBackground(Bundle... params) {
			...
			try {
				LinkedInApiClient apiClient = mLinkedInApiClientFactory
						.createLinkedInApiClient(mSharedPreferences.getString(
								SAVE_STATE_KEY_OAUTH_TOKEN, ""),
								mSharedPreferences.getString(
		...
		@Override
		protected void onPostExecute(Bundle result) {
			if (!handleRequestResult(result, REQUEST_POST_MESSAGE))
				return;

			((OnPostingCompleteListener) mLocalListeners
					.get(REQUEST_POST_MESSAGE)).onPostSuccessfully(getID());
		}
	}
	...
	private class RequestCheckIsFriendAsyncTask extends SocialNetworkAsyncTask {
		...
		protected void onPostExecute(Bundle result) {
			...
			((OnCheckIsFriendCompleteListener) mLocalListeners
					.get(REQUEST_CHECK_IS_FRIEND)).onCheckIsFriendComplete(
					getID(), result.getString(RESULT_REQUESTED_ID),
	...
	private class RequestSendInviteAsyncTask extends SocialNetworkAsyncTask {
		...
		protected Bundle doInBackground(Bundle... params) {
			...
			try {
				...
				String authHeader = "";
				Headers headers = person.getApiStandardProfileRequest()
						.getHeaders();
				List<HttpHeader> httpHeaders = headers.getHttpHeaderList();
				for (HttpHeader httpHeader : httpHeaders) {
					if (httpHeader.getName().equals(
							ApplicationConstants.AUTH_HEADER_NAME)) {
						authHeader = httpHeader.getValue();
						break;
					}
				}

				CommunicationsApiClient communicationsApiClient = mLinkedInApiClientFactory
						.createCommunicationsApiClient(mSharedPreferences
								.getString(SAVE_STATE_KEY_OAUTH_TOKEN, ""),
								mSharedPreferences.getString(
										SAVE_STATE_KEY_OAUTH_SECRET, ""));
		...
		protected void onPostExecute(Bundle result) {
			...
			((OnRequestAddFriendCompleteListener) mLocalListeners
					.get(REQUEST_ADD_FRIEND)).onRequestAddFriendComplete(
					getID(), result.getString(RESULT_REQUESTED_ID));
			mLocalListeners.remove(REQUEST_CHECK_IS_FRIEND);
		}
	}
